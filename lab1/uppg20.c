...
#define	PIXEL_PURPLE		0x400040
#define	PIXEL_YELLOW		0x00FFFF
	...
	for (i = 0; i < w; i++)
		for (j = 0; j < h; j++)
				if (img.data[(i + x) + (j + y) * img.w] == PIXEL_HIGHLIGHT) {
					pixbuf[i + j * w] = PIXEL_YELLOW;
					(*piczels)++;
				} else
					pixbuf[i + j * w] = PIXEL_PURPLE;
...
DARNIT_TILESHEET *detect_image(DARNIT_IMAGE_DATA img, int x, int y, int *piczels) {
	...
	fill_image(img, x, y, PIXEL_HIGHLIGHT, PIXEL_PURPLE);
...
int main(int argc, char **argv) {
	...
	d_render_clearcolor_set(64, 0, 64);
	do {
		i = 0;
		ts = next_image(img, &i);
		if (ts) {
